...
onlymemBcells %>%
  ...
  geom_histogram() +
  theme_classic()+
  labs(title = "Age distribution onlymemBcells", x="Age of patient at visit time", y="Number of patients")

  labs(title = "Distribution onlymemBcells", x="% of memory B cells", y="Number of patients")
...
#tellen welke donors niet in categorie gezet zijn met age function
tellNULLmem <- onlymemBcells %>% filter(age_group == "NULL")

#boxplot for %memroy B cells per age group (& colored with vaccine)
onlymemBcells %>%
  ggplot(aes(x=age_group, y=data, color=vaccine))+
  geom_boxplot() +
  theme_classic() +
